...
| SME-017 | 2026-06-16 | Christian (self, RT) | Bug | blocker | new | Switching VC→PC spuriously fires an APNEA alarm on a non-apneic patient (backup rate 14, measured RR 14). Reproduced with the LOW-VE alarm on VC→PC switch too. The high-pressure alarm CLEARING on the switch is correct physiology (PC defaults PIP 20, below threshold) — the false apnea/low-VE firing is NOT. | alarms.js / simulation.js / main.js (alarm evaluation + sim.reset timing) | HYPOTHESIS (unverified): possible cousin of SME-008 — sim.reset() zeroes sim.globalTime on mode switch; if apnea/VE detection is keyed to sim-time or a stranded last-breath timestamp, the reset may briefly read as "no recent breath" → false alarm. To be confirmed by read-only investigation before any fix. Highest priority — a false apnea alarm actively misleads learners. |
...
| Alarm bugs surfaced in deep testing | 1 (self, deep testing) | Bug+Usability | mixed | new | SME-017, SME-018, SME-019 | Found while stress-testing the alarm subsystem. False apnea/low-VE on mode switch is the blocker; silence-reset + medium tuning are usability. |
